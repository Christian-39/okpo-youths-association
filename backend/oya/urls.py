"""
URL configuration for OYA project.
"""

from django.contrib import admin
from django.urls import path, include
from django.shortcuts import redirect
from django.conf import settings
from django.conf.urls.static import static
from dashboard.views import global_search_ajax

urlpatterns = [
    path("admin/api/", admin.site.urls),
    path("", lambda request: redirect("accounts:login"), name="home"),
    path("", include("accounts.urls")),
    path("accounts/api/", include("accounts.urls")),
    path("members/api/", include("members.urls")),
    path("executives/api/", include("executives.urls")),
    path("elections/api/", include("elections.urls")),
    path("finance/api/", include("finance.urls")),
    path("projects/api/", include("projects.urls")),
    path("operations/api/", include("operations.urls")),
    path("notifications/api/", include("notifications.urls")),
    path("auditlogs/api/", include("auditlogs.urls")),
    path("dashboard/api/", include("dashboard.urls")),
    path("settings/api/", include("settingsapp.urls")),
    path("project-donations/api/", include("project_donations.urls")),
    
    path('search/api/', global_search_ajax, name='global_search_ajax'),
]

if settings.DEBUG:
    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)
    urlpatterns += static(settings.STATIC_URL, document_root=settings.STATIC_ROOT)
